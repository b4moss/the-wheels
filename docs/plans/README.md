...
チャットで PO が決めたことは、**その場では確定扱いにしない**。該当 `plans/`（CI・テストに効くものは `docs/tests/` / `git.md` / `test.md` も）へすぐ転記する。未実装 WC の `specs/` は作らない。転記したら `wishlist.md` の該当未決は消す。各版の実装着手は、その版の要決定が plans に落ちてから。
